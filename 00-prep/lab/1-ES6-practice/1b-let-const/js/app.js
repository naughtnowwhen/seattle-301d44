...
function displayPics(){
  ...
  // Because when using const, the constibale context leaks outside the function, i think this is called hoisting? But either way, when we switch from const to const, the context is confined to block scope, and therefore attempting to log it wont work because the outside scope is blind to it. it is a little strange that even a broken console.log can break the app. The pictures don't display with that console log when rando is 'letted';
...
function handleClick(event) {
  ...
  for(let i = 0; i < names.length; i++){
    if(event.target.id === allProducts[i].name) {
      ...
      console.log(`${event.target.id} has ${allProducts[i].votes} votes int ${allProducts[i].views} views`);
